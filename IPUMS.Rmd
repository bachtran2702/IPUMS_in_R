...
### Education Attainment v. various factors
...
ggplot(more_than_four_college_year_to_population_age_25_to_67, aes(YEAR, percent)) +
  ...
  labs(
    ...
    caption = paste(
      strwrap(ipums_var_desc(ddi, EDUCD), 110),
      collapse = "\n")) +
  ...
  theme(text=element_text(size=12,  family= font1)) #I split the function into two for readability
```
#_________________________________PLEASE DISREGARD ALL CODE BELLOW THIS LINE (FOR NOW)_________________________________
#### Composition of people with more than 4 years of college to population from 25-67 
``` {r overview graph}
ggplot(more_than_four_college_year_to_population_age_25_to_67, aes(YEAR, prop)) +
  ...
  labs(
    ...
    subtitle = paste0("Data source: ", ddi$ipums_project),
    caption = paste(
      strwrap(ipums_var_desc(ddi, EDUCD), 90),
      collapse = "\n"
    ) +
  coord_flip() 
  )
